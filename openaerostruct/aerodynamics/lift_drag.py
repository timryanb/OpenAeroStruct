...
class LiftDrag(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials["L", "sec_forces"] = np.atleast_2d(np.tile(tmp, self.num_panels)) * symmetry_factor

        tmp = np.array([cosa * cosb, -sinb, sina * cosb])
        partials["D", "sec_forces"] = np.atleast_2d(np.tile(tmp, self.num_panels)) * symmetry_factor

        partials["L", "alpha"] = p180 * symmetry_factor * np.sum(-forces[:, :, 0] * cosa - forces[:, :, 2] * sina)
        partials["D", "alpha"] = (
            p180 * symmetry_factor * np.sum(-forces[:, :, 0] * sina * cosb + forces[:, :, 2] * cosa * cosb)
        )

        partials["D", "beta"] = (
            p180
            * symmetry_factor
            * np.sum(-forces[:, :, 0] * cosa * sinb + -forces[:, :, 1] * cosb + -forces[:, :, 2] * sina * sinb)
        )
