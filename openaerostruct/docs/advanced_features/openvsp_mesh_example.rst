...
The discretization of the VLM is based on the tesselation refinement define in OpenVSP model.
To get an idea of what the discretization will look like,
users can turn on the wireframe model for each surface in OpenVSP.
The spanwise and chordwise lines will roughly correspond to panels in the exported VLM mesh.
More or less VLM panels can be added in the chordwise and spanwise direction for each surface by toggling
the following OpenVSP parameters.
