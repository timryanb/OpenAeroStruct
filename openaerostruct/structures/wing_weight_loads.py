...
class StructureWeightLoads(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        dbm3_dnodes = (
            diags(nym1_rand) * dzm_dnodes_pattern
        ...
        self.dbm3_dnodes_pattern = (
            diags(nym1_rand) * dzm_dnodes_pattern
            + diags(nym1_rand) * self.ddel1__dnodes
            - diags(nym1_rand) * self.del__dnodes
        )
        self.dbm4_dnodes_pattern = (
            diags(nym1_rand) * dzm_dnodes_pattern
            + diags(nym1_rand) * self.ddel0__dnodes
            - diags(nym1_rand) * self.del__dnodes
        )
        ...
        self.declare_partials(
            "struct_weight_loads", "nodes", rows=self.dswl__dnodes_pattern.row, cols=self.dswl__dnodes_pattern.col
        )

        self.declare_partials(
            "struct_weight_loads", "element_mass", rows=self.dswl__dew_pattern.row, cols=self.dswl__dew_pattern.col
        )
    ...
    def compute_partials(self, inputs, J):
        ...
        dswl__dlf = (
            -self.dswl__dbm3 * dbm3__dzm * coo_matrix(dzm__dlf).T
            + -self.dswl__dbm4 * dbm4__dzm * coo_matrix(dzm__dlf).T
            + self.dswl__dzf * coo_matrix(dzf__dlf).T
        ).tolil()

        J["struct_weight_loads", "load_factor"] = (
            dswl__dlf[self.dswl__dlf_row, self.dswl__dlf_col].toarray().flatten() * grav_constant
        )

        dswl__dew = (
            -self.dswl__dbm4 * dbm4__dzm * dzm__dew
            + -self.dswl__dbm3 * dbm3__dzm * dzm__dew
            + self.dswl__dzf * dzf__dew
        ).tolil()
