    ...
    def test(self):
        ...
        prob.model.add_subsystem(point_name, AS_point,
            promotes_inputs=['v', 'alpha', 'Mach_number', 're', 'rho', 'CT', 'R',
                'W0', 'speed_of_sound', 'empty_cg', 'load_factor','height_agl'])
        ...
        prob.run_driver()
        assert_rel_error(self, prob['AS_point_0.fuelburn'][0], 97696.33252514644, 1e-6)
        prob['height_agl'] = 20.
        prob.run_driver()
        # the fuel burn should be less in ground effect
        assert_rel_error(self, prob['AS_point_0.fuelburn'][0], 91994.52652058695, 1e-6)
        totals = prob.check_totals(of=['AS_point_0.L_equals_W','AS_point_0.fuelburn','AS_point_0.wing_perf.failure'], wrt=['wing.twist_cp','alpha','height_agl'], compact_print=True, out_stream=None)
